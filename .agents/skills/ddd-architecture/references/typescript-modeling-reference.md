...
type ValidateOrder = (input: UnvalidatedOrder) => Either<ValidationError, ValidatedOrder>;
...
```typescript
type CalculatePrices = (getProductPrice: GetProductPrice) => (input: OrderForm) => PricedOrder;
