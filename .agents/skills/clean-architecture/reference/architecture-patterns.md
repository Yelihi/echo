...
export const useSyncData = ({
  ...
  const convertedData = useMemo(() => {
    ...
    return infiniteData.pages.flatMap((page) => page.items).map((item) => converter.convert(item));
  }, [infiniteData]);
...
type PageStore = StoreState & {
  ...
  setFilters: <K extends keyof StoreState["filters"]>(
    key: K,
    value: StoreState["filters"][K],
  ) => void;
...
export const usePageStore = create<PageStore>((set) => ({
  ...
  resetFilters: () =>
    set({ filters: { keyword: undefined, category: null }, isFilterActive: false }),
}));
```
