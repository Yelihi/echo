...
create or replace function public.list_study_sessions(
  ...
  with sessions as (
    select id, user_id, material_id, material_title_snapshot as title, created_at, status::text as recording_status,
      ...
      (select count(*) from public.accepted_recordings a
        join public.roleplay_session_lines l on l.id = a.roleplay_line_id
        where a.roleplay_session_id = s.id and l.speaker_order = s.selected_learner_speaker_order) as saved_count
    from public.roleplay_sessions s
    where user_id = auth.uid() and status <> 'deleted' and deleted_at is null
    union all
    select id, user_id, material_id, material_title_snapshot, created_at, status::text, 'memorization',
      ...
      (select count(*) from public.accepted_recordings a where a.memorization_session_id = s.id)
    from public.memorization_sessions s
    where user_id = auth.uid() and status <> 'deleted' and deleted_at is null
  ), classified as (
    select s.*, case
      when s.recording_status <> 'completed' then 'practicing'
  ...
  select jsonb_build_object(
    ...
    'items', coalesce((select jsonb_agg(jsonb_build_object(
      ...
      -- Keep the existing analysis-state wire contract compatible with deployed clients.
      -- New clients derive practice state from recordingCompleted.
      'state', case when analysis_status = 'practicing' then 'pending' else analysis_status end,
      'sourceMaterialId', material_id, 'targetCount', target_count, 'savedCount', saved_count,
